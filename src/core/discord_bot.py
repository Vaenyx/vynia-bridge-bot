...
class DiscordBridgeBot(commands.Bot):
    mineflayer_bot: MinecraftBotManager | None
    ...
    async def ready_check(self, ctx: commands.Context[Any]) -> bool:
        return (
            self.is_ready()
            and self.mineflayer_bot is not None
            and self.mineflayer_bot.is_ready()
        )
    ...
    async def on_ready(self) -> None:
        ...
        if channel is None:
            print(
                f"Discord > Channel {DiscordConfig.channel} not found! "
                "Please set the correct channel ID!"
            )
            await self.close()
            return
    ...
    async def on_message(self, message: discord.Message) -> None:
        ...
        if self.mineflayer_bot is None or not self.mineflayer_bot.is_ready():
            await self.send_message(content="Minecraft bot is not ready yet.")
            return
        ...
        self.mineflayer_bot.send_minecraft_message(
    ...
    async def send_message(self, *args: Any, **kwargs: Any) -> discord.Message | None:
        ...
        try:
            return cast(discord.Message, await send(*args, **kwargs))
        except aiohttp.ClientError:
            return None

    async def send_user_message(self, username: str, message: str) -> discord.Message | None:
        return await self.send_message(
            content=f"G > {username}: {discord.utils.escape_markdown(message)}"
        )

    async def send_minecraft_user_message(
        self,
        username: str,
        ...
        if not content:
            if message.attachments:
                ...
                article = "a" if count == 1 else str(count)
                suffix = "s" if count > 1 else ""
                await self.mineflayer_bot.chat(
                    f"/gc {username}: attached {article} file{suffix}"
                )
            return
        ...
        if message.reference is not None and message.reference.message_id is not None:
            ...
            if message.reference.cached_message is not None:
                reply = cast(discord.Message, message.reference.cached_message)
            else:
                reply = None
                try:
            ...
            if reply.author != self.user:
                ...
                reply_to = None
                if reply.embeds and reply.embeds[0].description:
                    try:
                        reply_to = reply.embeds[0].author.name
                    except AttributeError:
                        reply_to = "unknown (bot probably)"
    ...
    async def send_discord_message(self, message: str) -> None:
        if not message:
            return

        if self.mineflayer_bot is not None and f"{self.mineflayer_bot.bot.name}:" in message:
            return
